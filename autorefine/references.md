...
### Dimension inference (pre-Phase 4)

Before Phase 4 defines formal dimensions, Phase 3 infers lightweight dimensions from observable fixture properties:
- **Input length:** character count bucketed into short (<500 chars), medium (500-2000), long (>2000)
- **Fixture source:** check Step 1 origin — was it generated by autorefine, provided by user, or synthetic?
- **Planted flaw:** check Step 1 fixture spec — was a deliberate flaw planted?

These are weaker than Phase 4 dimensions but sufficient for first-pass stratification. On re-runs (after Phase 4 completes), use the richer dimensions defined there instead.

### Consistency detection algorithm

After each human judgment (starting at review #5):
1. Get the cluster ID of the just-reviewed trace
2. Scan all previously reviewed traces for the same cluster ID
3. If any share a cluster but received a different Pass/Fail verdict, flag it
4. Complexity: O(n) per judgment — scan the reviewed list once. Acceptable for n<=10.

### When to override consistency flags

Not every flag requires changing a verdict. Same-cluster traces can legitimately get different verdicts if one is borderline. The flag's purpose is to prompt reflection, not enforce consistency. If the user confirms both verdicts after reviewing, move on.
