...
- `edit_budget`: null, or `{"schedule":"constant","max_edits":N,"current_budget":N}`. Bounds how many discrete changes (distinct `##` sections added, removed, or modified versus the baseline) a single Phase 7 mutation experiment may make: the textual learning-rate analogue. It is applied as a **generation-time constraint** (the proposer makes at most `current_budget` changes when writing `candidate_skill_revision.content`); there is no post-hoc clip, because the candidate is a full `content` rewrite, not an edit list. A legacy v2/v3 workspace predating this field is treated as unbounded until a fresh Phase 7 start initializes it. `max_edits` must be `>= 1`. `schedule` must be `"constant"` (the active default for v4.x, where `current_budget` always equals `max_edits`) or `"unbounded"` (regularizer disabled — no cap, `max_edits`/`current_budget` ignored); reject any other value at load (the `cosine` shape is reserved but not implemented). Phase 7 iteration-start initializes `edit_budget` to `{"schedule":"constant","max_edits":3,"current_budget":3}` when it is null, so the regularizer is active by default (surgical edits — keeps one mutation from rewriting large regions and overfitting to a single failure); iteration-start must NOT overwrite a pre-set value. To use a different cap, set a higher `max_edits`; to disable the regularizer for a run, set `edit_budget = {"schedule":"unbounded"}` before Phase 7 start (null no longer disables — it triggers default initialization). On every load/resume, for a `constant` schedule recompute `current_budget = max_edits` so a stale or hand-edited `current_budget` cannot silently cap mutations differently from the configured `max_edits`. The mutation step records `mutation.md > edit_budget_applied = {current_budget, change_count}`; the circuit breaker reads `edit_budget` plus those records before diagnosing a content ceiling so a too-tight budget is not mistaken for exhausted content.
...
- `edit_budget_applied`: null when `state.json.edit_budget` is null or `schedule = "unbounded"`; otherwise `{current_budget, change_count}` recording the active `current_budget` and the number of discrete `##`-section changes the candidate made versus the baseline (the canonical count, computed by comparing candidate `content` to the baseline body — not the later human-facing `changes[]` summary). The Phase 7 circuit-breaker budget guard reads this to distinguish a too-tight budget from exhausted content.
...
- Initialize `state.json.edit_budget` in the same run-start write: when null, set the default `{"schedule":"constant","max_edits":3,"current_budget":3}` (active-by-default bounded-edit regularizer); do not overwrite a pre-set value such as `{"schedule":"unbounded"}`. Validate `schedule ∈ {constant, unbounded}` and `max_edits >= 1`. See the `edit_budget` field doc for full semantics.
