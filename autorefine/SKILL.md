...
If workspace exists **with** `state.json`: read it, deserialize any persisted `phase1_context` (including `selected_skill_pattern` and `selected_eval_strategy_id`) plus any persisted `mutation_stage_split_access_policy`, `iteration_state`, `mid_session_preference_signals`, and `mid_session_preference_signals_path`, `selected_adapter_id`, `adapter_config_path`, and `active_experiment_contract_path` into the loaded run context, then normalize the active-loop `style_preferences` payload using `references.md > Style Preferences Payload` before printing pipeline status.
If workspace exists **with** `state.json`: read it, deserialize any persisted `phase1_context` into the loaded run context, and print pipeline status.
...
Deserialize `state.json.phase1_context`, `state.json.mutation_stage_split_access_policy`, `state.json.iteration_state`, `state.json.selected_adapter_id`, `state.json.adapter_config_path`, and `state.json.active_experiment_contract_path` into the loaded run context before routing or resuming later phases.
deserialize `state.json.phase1_context`, `state.json.mutation_stage_split_access_policy`, `state.json.selected_adapter_id`, `state.json.adapter_config_path`, and `state.json.active_experiment_contract_path` into the loaded run context before routing the resume path.
deserialize `state.json.phase1_context`, `state.json.mutation_stage_split_access_policy`, `state.json.iteration_state`, `state.json.selected_adapter_id`, `state.json.adapter_config_path`, and `state.json.active_experiment_contract_path` into the loaded run context.
Restore `phase1_context.selected_skill_pattern` unchanged so later phases can read the chosen pattern from the loaded context rather than recomputing it.
Then clear the checkpoint (set to null) while preserving every other serialized state field, including `phase1_context`.
