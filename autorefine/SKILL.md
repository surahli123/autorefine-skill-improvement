...
If workspace exists **with** `state.json`: run **Run Context Recovery** below before printing pipeline status.

### Run Context Recovery (canonical contract)

Run this contract once at startup/resume, before Gulf routing or phase-specific instructions. It is the only canonical restore/hydration contract; Gulf support files may add narrow phase-local gates, but they must not redefine workspace hydration.

1. Read `[workspace]/state.json`. Legacy workspaces (`schema_version` 2 or 3) are read-compatible; missing newer fields default to null.
2. Hydrate the loaded run context from these persisted fields when present: `contract_status`, `contract_path`, `effectiveness_floor`, `phase1_context`, `mutation_stage_split_access_policy`, `iteration_state`, `mid_session_preference_signals`, `mid_session_preference_signals_path`, `selected_adapter_id`, `adapter_config_path`, `domain_eval_config_path`, `active_experiment_contract_path`, and `meta_learnings_path`.
3. Prefer `state.json.adapter_config_path` as the canonical runtime adapter pointer. If it is null and `state.json.domain_eval_config_path` is populated, hydrate `adapter_config_path` from that legacy alias for the loaded run context. Preserve both persisted state keys unchanged on writes unless a phase explicitly owns updating them.
4. Restore `phase1_context.selected_skill_pattern` and `phase1_context.selected_eval_strategy_id` unchanged. If the restored pattern and `state.json.skill_pattern` mismatch, stop and rerun Phase 1 Step 0. If the restored strategy is missing or no longer maps back to the restored pattern through `references.md > Skill Pattern Eval Strategy > Pattern-to-Evaluation-Strategy Selector`, stop and rerun strategy selection before downstream scoring.
5. If split-scoped Phase 7 work is active and `mutation_stage_split_access_policy` is missing, read the same policy from `fixtures-manifest.md` or a stored Phase 4 `evaluation_metadata.config.mutation_stage_split_access_policy` snapshot, hydrate the loaded run context, and stop if those sources disagree.
6. If `iteration_state` is present, treat it as the authoritative Phase 7 handoff for whether the active `run_id` is in eval, mutate, test, or session_close. Continue automatic progression from its persisted `next_action` until terminal success (`phase_status = "completed"`) or terminal failure (`phase_status = "blocked"`). Do not infer boundaries from directory scans while the persisted runner state is available.
7. Rebuild the normalized `style_preferences` payload from `mid_session_preference_signals` plus `mid_session_preference_signals_path` using `references.md > Style Preferences Payload` before any Phase 7 or Session Close mutation steering.
8. If the session may enter Phase 7 or Session Close, bootstrap meta-learnings from `state.json.meta_learnings_path` or the default AutoRefine skill copy using `references.md > Campaign Bootstrap Meta-Learnings Context`. Rebuild this object on every start/resume; do not persist parsed entries into `state.json`.
9. If `state.json.checkpoint` is not null and has `next_action`, enter resume mode. Read all files in `checkpoint.files_to_read_on_resume` (skip missing files and note them), print `Resuming from checkpoint: {next_action}`, run the integrity checks below, then clear `state.json.checkpoint` to null while preserving every other serialized state field.
10. If `state.json.checkpoint` is null, still run the adapter and contract integrity checks when their active state says they must be trusted before downstream routing.

**Contract integrity check.** If `contract_status = "confirmed"`, validate the full contract artifact set before trusting downstream state. Check all 4 required files exist and are non-empty:
...
On any contract validation failure, stop with a blocking error naming the missing or malformed file(s):
...
Fail closed. Do not silently reset `contract_status` to `"skipped"` or continue with partial data. If validation passes, log `{"phase":"checkpoint","type":"contract_integrity_check","status":"passed","files_validated":4}` to `session-log.json`.

**Adapter evidence integrity check.** Validate the canonical adapter config path if set (`state.json.adapter_config_path`, or the legacy alias `state.json.domain_eval_config_path` when `adapter_config_path` is null) with a full file integrity check, not just config existence:
...
Fail closed. Do not silently clear adapter config fields to null. The user must explicitly choose. Log `{"phase":"checkpoint","type":"domain_eval_integrity_check","status":"passed|failed","files_validated":2}` to `session-log.json`.

After a checkpoint resume succeeds, rotate `session-log.json` (rename to `session-log-<session_start, colons->dashes>.json`, create fresh). If `session-log.json` is missing (pre-v2 workspace), create it. If checkpoint has `next_action` pointing to a Phase 7 experiment, **skip ambient learning entirely** because the workspace copy must match the in-progress experiment state, and proceed from `next_action`.

**Pattern-aware downstream entry:** When routing into any downstream phase or stage after Phase 1, use the restored `state.json.phase1_context.selected_skill_pattern` and `state.json.phase1_context.selected_eval_strategy_id` from Run Context Recovery as the active downstream routing state. If the active context does not already hold them, read the same canonical IDs from the top-level `selected_skill_pattern` and `selected_eval_strategy_id` fields emitted in `design-audit.md`, hydrate the loaded run context, and continue. If only the pattern is available, resolve the missing strategy through `references.md > Skill Pattern Eval Strategy > Pattern-to-Evaluation-Strategy Selector` before continuing. After hydrating `selected_eval_strategy_id`, immediately open the matching row in `references.md > Skill Pattern Eval Strategy > Strategy Definitions` and route all downstream eval work through that strategy bundle. The selected strategy is the execution path for Quick Start bootstrap evals, Phase 2 eval audit, Phase 3/4 failure clustering and fixture expansion, Phase 5/6 judge design, and Phase 7 mutation analysis; do not fall back to the generic downstream path while a valid selector is present. Do not trigger Phase 1 pattern classification again during downstream phase/stage initialization; rerun Phase 1 Step 0 only if the persisted pattern is missing or inconsistent with `state.json.skill_pattern`.
...
Phase 7 writes the Aggregation Explainer and persists `decision_breakdown`, including `components`, `weight_source`, `normalized_contribution`, `combined_score`, and the keep/discard threshold. As soon as scoring completes for the experiment, populate `decision_breakdown` with the intermediate per-eval math before user presentation.
As soon as scoring completes for the experiment, populate `decision_breakdown` with the intermediate per-eval math and the final aggregate used for the keep/discard threshold.
