"""Campaign planning and report assembly for AutoRefine Phase 7 work."""
...
from .campaign_readiness import assess_campaign_readiness
...
def build_target_stage_plan(target: dict) -> list[dict]:
    return assess_campaign_readiness(target).to_stage_dicts()
...
def build_campaign_report_from_manifest(
    manifest: dict,
    *,
    base_dir: Path | None = None,
) -> dict:
    return campaign_report(validate_campaign_manifest(manifest, base_dir=base_dir))
