...
**Adapter-aware fixture handoff (generic):** If `state.json.selected_adapter_id` is present, every fixture used downstream by the adapter-aware path must preserve enough information for the adapter's `input_schema` and `gold_source` from `references.md > Domain Adapter Contract Schema`. Do not invent domain-specific fixture formats inside Phase 4 itself. Instead, make sure the fixture payload can be converted into the adapter's canonical input shape and keep stable `input_id`s so the evaluator can score the same items later.

**Search adapter specialization:** If `state.json.selected_adapter_id = "search_retrieval_v1"`, preserve retrieval-specific fixture identity explicitly:
- each fixture must keep the user query verbatim
- each labeled target result must expose stable `doc_id`s
- any ranking labels or relevance grades must survive fixture expansion unchanged

Do not reduce search fixtures to prose-only expected outputs. The evaluator needs ranked-result identity, not just a natural-language success description.
...
If `state.json.selected_adapter_id` is present, also split the evaluation surface into:
- **primary oracle** — the adapter-specific task-truth metric
- **secondary judges** — behavioral or presentation checks that diagnose quality and catch regressions

The primary oracle decides task quality for the adapter-aware path. Secondary judges do not replace it.

For `search_retrieval_v1`:
...
If both `state.json.adapter_config_path` and the legacy alias `state.json.domain_eval_config_path` are null: skip this eval class entirely (no domain-metric eval in eval-suite.md). If either config path is set but the golden-set file is missing or empty: write the eval definition to `eval-suite.md` but mark it `inactive_pending_golden_set: true` so it's surfaced but not scored until the author provides labeled data.
...
**Step 4b: Confound check (required before accepting high TPR/TNR).** High agreement is NOT sufficient evidence of a specific judge. A shortcut judge that keys on a **surface correlate** (length, section headers/keywords, formatting) can score 100/100 when the good and degraded piles differ on *everything* at once — as the original good-vs-degraded set did (a pure length-detector also scored it 100/100). Before the Gulf 2 gate may be APPROVED, execute this checklist:

- **(a) List surface correlates.** For THIS skill's failure taxonomy, list every candidate surface correlate a shortcut judge could exploit — at minimum: **length**, **section headers/keywords**, and **formatting/structure**. Add any others specific to the skill's outputs.
- **(b) Build one minimal pair per correlate.** For each listed correlate, construct a **length-matched positive/negative minimal pair** that holds that correlate constant while flipping the true label (e.g. a SHORT output WITH the real defect vs a LONG output WITHOUT it, so length cannot decide; a doc WITH the keyword/header but the wrong content vs one WITHOUT the header but the right content). Run the judge on the pairs: a genuine judge tracks the true label, a shortcut judge tracks the correlate.
- **(c) Treat a near-perfect first pass as a RED FLAG.** A first-pass judge score at or above **95/95** — and especially **100/100** — is a **RED FLAG**, not evidence of a good judge. Do NOT approve the gate on a near-perfect score until the minimal pairs in (b) show the judge tracks the true label and not a surface correlate.
