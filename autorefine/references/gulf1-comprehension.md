...
Phase-entry contract: first run `SKILL.md > Run Context Recovery`. For Quick Start QS Step 2-4, Phase 2, or Phase 3, use the recovered `phase1_context.selected_skill_pattern` and `phase1_context.selected_eval_strategy_id` as the active downstream route. If only `design-audit.md` is available, hydrate the same IDs from its top-level fields; if only the pattern is available, resolve the missing strategy through `references.md > Skill Pattern Eval Strategy > Pattern-to-Evaluation-Strategy Selector`. Do not rerun Phase 1 Step 0 unless the persisted pattern is missing or mismatched with `state.json.skill_pattern`.
...
2. If non-null, check the canonical adapter config pointer first:
   ...
   - If either is already set (resume case): run `SKILL.md > Run Context Recovery > Adapter evidence integrity check` before trusting it. If the check passes, keep both fields aligned and skip Step 7 (domain eval already configured correctly). If the check fails (file missing, corrupted, or author_confirmed=false), stop with the canonical recovery options. Re-enter Step 7 only after the user explicitly chooses to clear BOTH `state.json.adapter_config_path` and `state.json.domain_eval_config_path` to null and reconfigure.
